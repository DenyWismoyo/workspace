...
pptx.title = 'Solo Technopark: Dari Kawasan Sains & Teknologi Menuju Mesin Impact Daerah (V6 - Executive Titanium Edition)';
pptx.subject = 'Bahan Paparan Resmi Walikota Surakarta 2026–2030 (Edisi Eksekutif V6)';
...
function addHeader(slide, title, category, slideNum, subtitle = '') {
  ...
  slide.addText('UPTD KST SOLO TECHNOPARK  |  BRIDA KOTA SURAKARTA  •  Bahan Paparan Resmi Walikota (Edisi V6)', {
...
statPills.forEach((st, sidx) => {
  ...
  s1.addShape(pptx.shapes.ROUNDED_RECTANGLE, {
    ...
    fill: C_DARK_BG, line: { color: st.col, width: 1 }, rectRadius: 0.04
  });
  s1.addText(`${st.label}: `, {
...
gapItems.forEach((g, gidx) => {
  ...
  s7.addShape(pptx.shapes.ROUNDED_RECTANGLE, {
    ...
    fill: g.isPriority ? '78350F' : '3F1515',
  ...
  s7.addShape(pptx.shapes.ROUNDED_RECTANGLE, {
    ...
    fill: g.isPriority ? '0F2338' : C_BENTO_BG2,
...
addBentoCard(s7, 0.8, 5.95, 11.733, 0.90, { fill: '0B1F38', line: C_AMBER, lineWidth: 1.2 });
...
s7.addText('Target: Membuka captive market Rp 2,25 Miliar serapan belanja OPD Pemkot Surakarta di 2030 bagi 80+ produk tenant dan startup binaan STP tanpa perlu alokasi pagu belanja baru di APBD. Hanya butuh 1 lembar Instruksi Walikota.', {
  ...
  fontSize: 8.8, color: C_WHITE, fontFace: FONT_BODY, lineSpacing: 12
});
...
s11.addChart(pptx.charts.BAR, impactChartData, {
  ...
  valAxisMaxVal: 110,
  showLegend: false,
  catGridLine: { color: '1E3A5F', size: 1 },
...
// WRITE TO DISK (V6)
// =========================================================================
const outputPptxPathV6 = path.resolve(__dirname, 'PAPARAN_WALIKOTA_STP_V6.pptx');
const outputPptxPathOut = path.resolve(__dirname, '../output_paparan/PAPARAN_WALIKOTA_STP_V6.pptx');

pptx.writeFile({ fileName: outputPptxPathV6 })
  .then(() => {
    console.log(`[SUCCESS] PPTX Master V6 generated at: ${outputPptxPathV6}`);
    fs.copyFileSync(outputPptxPathV6, outputPptxPathOut);
    console.log(`[SUCCESS] Copied to output_paparan: ${outputPptxPathOut}`);
  })
  .catch(err => {
    console.error('[ERROR] Failed generating PPTX V6:', err);
    process.exit(1);
  });
