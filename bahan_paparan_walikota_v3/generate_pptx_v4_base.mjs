...
pptx.title = 'Solo Technopark: Dari Kawasan Sains & Teknologi Menuju Mesin Impact Daerah (V4 - Visual Diagrams)';
...
// =========================================================================
// DIAGRAM HELPERS (V4 Visual Upgrade)
// =========================================================================
function addBarH(slide, x, y, maxW, value, maxValue, color) {
  const barW = Math.max((value / maxValue) * maxW, 0.05);
  slide.addShape(pptx.shapes.ROUNDED_RECTANGLE, { x, y, w: maxW, h: 0.28, fill: { color: '0D213F' }, line: { color: '1E3A5F', width: 1 }, rectRadius: 0.04 });
  slide.addShape(pptx.shapes.ROUNDED_RECTANGLE, { x, y, w: barW, h: 0.28, fill: { color: color }, line: { color: color }, rectRadius: 0.04 });
  return barW;
}
function addProgressBar(slide, x, y, maxW, pct, color) {
  const barW = Math.max((pct / 100) * maxW, 0.05);
  slide.addShape(pptx.shapes.ROUNDED_RECTANGLE, { x, y, w: maxW, h: 0.14, fill: { color: '0D213F' }, line: { color: '1E3A5F', width: 1 }, rectRadius: 0.04 });
  slide.addShape(pptx.shapes.ROUNDED_RECTANGLE, { x, y, w: barW, h: 0.14, fill: { color: color }, line: { color: color }, rectRadius: 0.04 });
}
function addHArrow(slide, x, y, len, color) {
  slide.addShape(pptx.shapes.RECTANGLE, { x, y: y + 0.04, w: len * 0.7, h: 0.08, fill: { color: color } });
  slide.addShape(pptx.shapes.RECTANGLE, { x: x + len * 0.65, y, w: 0.08, h: 0.28, fill: { color: color } });
}
...
// ── VISUAL CHAIN OF IMPACT FLOW DIAGRAM ─────────────────────────────────
s2.addText('RANTAI DAMPAK KELEMBAGAAN (CHAIN OF IMPACT)', {
  x: 0.8, y: 1.35, w: 11.733, h: 0.25,
  fontSize: 9, bold: true, color: C_AMBER, fontFace: FONT_HEAD, align: 'center'
});
// Node 1: BRIDA
addBentoCard(s2, 0.65, 1.62, 3.2, 1.02, { fill: C_BENTO_BG2, line: C_CYAN, lineWidth: 2 });
s2.addShape(pptx.shapes.RECTANGLE, { x: 0.65, y: 1.62, w: 3.2, h: 0.07, fill: { color: C_CYAN } });
s2.addText('BRIDA\nKOTA SURAKARTA', { x: 0.65, y: 1.71, w: 3.2, h: 0.45, fontSize: 11, bold: true, color: C_WHITE, fontFace: FONT_HEAD, align: 'center', lineSpacing: 14 });
s2.addText('Otak & Regulator Kebijakan', { x: 0.65, y: 2.15, w: 3.2, h: 0.22, fontSize: 8.5, bold: true, color: C_CYAN_LIGHT, fontFace: FONT_BODY, align: 'center' });
// Arrow 1
addHArrow(s2, 3.85, 2.07, 0.5, C_CYAN);
s2.addText('Regulasi & Arahan', { x: 3.82, y: 1.82, w: 0.56, h: 0.32, fontSize: 6.5, color: C_CYAN_LIGHT, fontFace: FONT_BODY, align: 'center' });
// Node 2: UPTD KST STP
addBentoCard(s2, 4.35, 1.62, 3.2, 1.02, { fill: C_BENTO_BG2, line: C_AMBER, lineWidth: 2 });
s2.addShape(pptx.shapes.RECTANGLE, { x: 4.35, y: 1.62, w: 3.2, h: 0.07, fill: { color: C_AMBER } });
s2.addText('UPTD KST\nSOLO TECHNOPARK', { x: 4.35, y: 1.71, w: 3.2, h: 0.45, fontSize: 11, bold: true, color: C_WHITE, fontFace: FONT_HEAD, align: 'center', lineSpacing: 14 });
s2.addText('Tangan Eksekutor & Delivery Unit', { x: 4.35, y: 2.15, w: 3.2, h: 0.22, fontSize: 8.5, bold: true, color: C_AMBER, fontFace: FONT_BODY, align: 'center' });
// Arrow 2
addHArrow(s2, 7.55, 2.07, 0.5, C_AMBER);
s2.addText('Layanan & Inovasi', { x: 7.52, y: 1.82, w: 0.56, h: 0.32, fontSize: 6.5, color: C_AMBER, fontFace: FONT_BODY, align: 'center' });
// Node 3: WARGA SOLO & INDUSTRI
addBentoCard(s2, 8.05, 1.62, 4.228, 1.02, { fill: '063A28', line: C_EMERALD, lineWidth: 2 });
s2.addShape(pptx.shapes.RECTANGLE, { x: 8.05, y: 1.62, w: 4.228, h: 0.07, fill: { color: C_EMERALD } });
s2.addText('WARGA SOLO\n& DUNIA USAHA', { x: 8.05, y: 1.71, w: 4.228, h: 0.45, fontSize: 11, bold: true, color: C_WHITE, fontFace: FONT_HEAD, align: 'center', lineSpacing: 14 });
s2.addText('Penerima Manfaat Utama (Benefisiari)', { x: 8.05, y: 2.15, w: 4.228, h: 0.22, fontSize: 8.5, bold: true, color: C_EMERALD, fontFace: FONT_BODY, align: 'center' });
...
stages.forEach((st, idx) => {
  ...
  if (idx < 4) {
    const arX = sx + 2.22;
    const arY = 2.5;
    s5.addShape(pptx.shapes.RECTANGLE, { x: arX, y: arY, w: 0.12, h: 0.08, fill: { color: st.accent } });
    s5.addShape(pptx.shapes.RECTANGLE, { x: arX + 0.1, y: arY - 0.1, w: 0.07, h: 0.28, fill: { color: st.accent } });
  }
  // Add numbered circle badge on top of each stage card
  s5.addShape(pptx.shapes.OVAL, {
    x: sx + 0.82, y: 1.3, w: 0.44, h: 0.44,
    fill: { color: st.accent }, line: { color: st.accent }
  });
  s5.addText(st.num, {
    x: sx + 0.82, y: 1.3, w: 0.44, h: 0.44,
    fontSize: 10, bold: true, color: C_DARK_BG, fontFace: FONT_HEAD, align: 'center', valign: 'middle'
  });
});
...
const tlY = 1.62;
s8.addShape(pptx.shapes.RECTANGLE, { x: 0.8, y: tlY, w: 11.733, h: 0.08, fill: { color: C_BORDER } });
// Phase 1 segment
s8.addShape(pptx.shapes.RECTANGLE, { x: 0.8, y: tlY, w: 3.6, h: 0.08, fill: { color: C_CYAN } });
s8.addShape(pptx.shapes.OVAL, { x: 0.68, y: tlY - 0.12, w: 0.24, h: 0.24, fill: { color: C_CYAN }, line: { color: C_CYAN } });
s8.addText('2026 - 2027', { x: 0.8, y: tlY - 0.38, w: 3.6, h: 0.28, fontSize: 8.5, bold: true, color: C_CYAN, fontFace: FONT_HEAD, align: 'center' });
// Phase 2 segment
s8.addShape(pptx.shapes.RECTANGLE, { x: 4.55, y: tlY, w: 3.8, h: 0.08, fill: { color: C_AMBER } });
s8.addShape(pptx.shapes.OVAL, { x: 4.43, y: tlY - 0.12, w: 0.24, h: 0.24, fill: { color: C_AMBER }, line: { color: C_AMBER } });
s8.addText('2028 - 2029', { x: 4.55, y: tlY - 0.38, w: 3.8, h: 0.28, fontSize: 8.5, bold: true, color: C_AMBER, fontFace: FONT_HEAD, align: 'center' });
// Phase 3 segment
s8.addShape(pptx.shapes.RECTANGLE, { x: 8.5, y: tlY, w: 3.933, h: 0.08, fill: { color: C_EMERALD } });
s8.addShape(pptx.shapes.OVAL, { x: 8.38, y: tlY - 0.12, w: 0.24, h: 0.24, fill: { color: C_EMERALD }, line: { color: C_EMERALD } });
s8.addText('2029 - 2030', { x: 8.5, y: tlY - 0.38, w: 3.933, h: 0.28, fontSize: 8.5, bold: true, color: C_EMERALD, fontFace: FONT_HEAD, align: 'center' });
// End marker
s8.addShape(pptx.shapes.OVAL, { x: 12.36, y: tlY - 0.12, w: 0.24, h: 0.24, fill: { color: C_AMBER }, line: { color: C_AMBER } });
...
phases.forEach((ph, idx) => {
  ...
  addBentoCard(s8, px, 1.85, 3.8, 3.6, { fill: C_BENTO_BG, accentColor: ph.accent });
  // Vertical connector from timeline to card
  s8.addShape(pptx.shapes.RECTANGLE, { x: px + 1.8, y: 1.68, w: 0.04, h: 0.2, fill: { color: ph.accent } });
...
// ── VISUAL BAR CHART (Left Column) ──────────────────────────────────────
addBentoCard(s9, 0.8, 1.45, 7.3, 4.6, { fill: C_BENTO_BG, line: C_BORDER });
s9.addText('GRAFIK PERTUMBUHAN PENDAPATAN BLUD 2026-2030 (dalam Rp Miliar)', {
  x: 1.0, y: 1.6, w: 6.9, h: 0.28, fontSize: 9.5, bold: true, color: C_CYAN_LIGHT, fontFace: FONT_HEAD
});

const barData9 = [
  { year: '2026', val: 7.50, pct: 'Baseline', color: C_SILVER },
  { year: '2027', val: 12.53, pct: '+67%', color: C_CYAN_LIGHT },
  { year: '2028', val: 16.10, pct: '+28.5%', color: C_CYAN },
  { year: '2029', val: 20.62, pct: '+28%', color: C_EMERALD },
  { year: '2030', val: 23.47, pct: 'MANDIRI', color: C_AMBER }
];
const barMaxW9 = 4.5;
const barBaseX9 = 2.4;
barData9.forEach((b, idx) => {
  const by = 2.02 + idx * 0.68;
  const bw = addBarH(s9, barBaseX9, by + 0.07, barMaxW9, b.val, 24, b.color);
  s9.addText(b.year, { x: 0.9, y: by, w: 1.35, h: 0.42, fontSize: 10, bold: true, color: C_WHITE, fontFace: FONT_HEAD, align: 'right', valign: 'middle' });
  s9.addText('Rp ' + b.val + ' M', { x: barBaseX9 + bw + 0.08, y: by, w: 1.2, h: 0.42, fontSize: 8.5, bold: true, color: b.color, fontFace: FONT_HEAD, valign: 'middle' });
  s9.addText(b.pct, { x: barBaseX9 + barMaxW9 + 1.4, y: by, w: 1.1, h: 0.42, fontSize: 8.5, bold: true, color: b.color, fontFace: FONT_HEAD, align: 'center', valign: 'middle' });
});
addBentoCard(s9, 1.0, 5.48, 6.9, 0.42, { fill: '0B2540', line: C_AMBER });
s9.addText([
  { text: 'TOTAL PERTUMBUHAN 2026-2030:  ', options: { bold: true, color: C_WHITE, fontSize: 10 } },
  { text: '+212,8%  (3,1x LIPAT)', options: { bold: true, color: C_AMBER, fontSize: 13 } },
  { text: '  -- BEP Tercapai di 2028', options: { color: C_SILVER, fontSize: 9 } }
], { x: 1.1, y: 5.48, w: 6.7, h: 0.42, fontFace: FONT_HEAD, align: 'center', valign: 'middle' });

// Left Bottom: Strategic Financial Conclusion (bar chart already rendered above)
...
impacts.forEach((im, idx) => {
  ...
  addBentoCard(s11, ix, iy, 3.8, 2.45, { fill: C_BENTO_BG, accentColor: im.accent });
  ...
  // Metric display - single clean centered block
  s11.addText(im.metric, {
    x: ix + 0.15, y: iy + 1.18, w: 3.5, h: 0.52,
    fontSize: 15, bold: true, color: im.accent, fontFace: FONT_HEAD, align: 'center', valign: 'middle'
  });
  // Progress bar
  addProgressBar(s11, ix + 0.2, iy + 1.75, 3.4, im.progress, im.accent);
  s11.addText(im.progress + '% Target 2030', {
    x: ix + 0.2, y: iy + 1.9, w: 3.4, h: 0.2,
    fontSize: 7.5, color: im.accent, fontFace: FONT_BODY, align: 'right', bold: true
  });
  s11.addText(im.submetric, {
    x: ix + 0.2, y: iy + 2.13, w: 3.4, h: 0.22,
...
// WRITE TO DISK
// =========================================================================
const outputPptxPathV4 = path.resolve(__dirname, 'PAPARAN_WALIKOTA_STP_V4.pptx');
const outputPptxPathOut = path.resolve(__dirname, '../output_paparan/PAPARAN_WALIKOTA_STP_V4.pptx');

pptx.writeFile({ fileName: outputPptxPathV4 })
  .then(() => {
    console.log(`[SUCCESS] PPTX Master V3 generated at: ${outputPptxPathV4}`);
    fs.copyFileSync(outputPptxPathV4, outputPptxPathOut);
