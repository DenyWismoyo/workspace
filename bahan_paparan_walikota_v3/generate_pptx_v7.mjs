...
pptx.title = 'Solo Technopark: Dari Kawasan Sains & Teknologi Menuju Mesin Impact Daerah (V7 - Sovereign Boardroom Light Edition)';
pptx.subject = 'Bahan Paparan Resmi Walikota Surakarta 2026–2030 (Edisi Eksekutif V7 - Sovereign Light Edition)';

// =========================================================================
// COLOR PALETTE (Executive Deep Navy & Bento Styling)
// =========================================================================
const C_DARK_BG     = 'F8FAFC'; // Sovereign Platinum Pearl Canvas
const C_BENTO_BG    = 'FFFFFF'; // Crisp Porcelain White Bento Card
const C_BENTO_BG2   = 'F1F5F9'; // Soft Slate Accent Card Fill
const C_BENTO_ALT   = 'F8FAFC'; // Alternating Table Fill
const C_BORDER      = 'CBD5E1'; // Refined Slate Border
const C_BORDER_LIGHT = 'E2E8F0'; // Hairline Border
const C_CYAN        = '0284C7'; // Sovereign Sky Blue accent
const C_CYAN_LIGHT  = '0369A1'; // Deep Sapphire Blue
const C_AMBER       = 'D97706'; // Imperial Gold Amber
const C_AMBER_LIGHT = 'B45309'; // Deep Amber text
const C_AMBER_BG    = 'FEF3C7'; // Soft Amber background tint
const C_EMERALD     = '059669'; // Forest Emerald
const C_EMERALD_BG  = 'ECFDF5'; // Soft Emerald tint
const C_PURPLE      = '7C3AED'; // Royal Purple accent
const C_PINK        = 'DB2777'; // Pink accent
const C_RED         = 'DC2626'; // Gap / Alert Red
const C_RED_BG      = 'FEE2E2'; // Soft Red tint
const C_WHITE       = '0F172A'; // Inverted for light theme: Primary Deep Slate text!
const C_SILVER      = '334155'; // Secondary slate text
const C_MUTED       = '64748B'; // Muted cool grey text
const C_NAVY_BAR    = '0F2942'; // Dark Navy for authoritative table/header bars
const C_BLUE_ACCENT = '1D4ED8'; // Royal Blue accent
const C_PURE_WHITE  = 'FFFFFF'; // Pure white when on dark backgrounds/badges
...
function addHeader(slide, title, category, slideNum, subtitle = '') {
  ...
  slide.addShape(pptx.shapes.ROUNDED_RECTANGLE, {
    ...
    fill: { color: 'E0F2FE' }, line: { color: C_CYAN, width: 1 }, rectRadius: 0.05
  });
  ...
  slide.addText('UPTD KST SOLO TECHNOPARK  |  BRIDA KOTA SURAKARTA  •  Bahan Paparan Resmi Walikota (Edisi V7 Light)', {
...
// =========================================================================
// SLIDE 01: COVER & FRAMING VISI BESAR (V6 Elevated Executive Masthead)
// =========================================================================
...
s1.addShape(pptx.shapes.ROUNDED_RECTANGLE, {
  x: 0.9, y: 0.55, w: 4.6, h: 0.36,
  fill: { color: C_BENTO_BG }, line: { color: C_CYAN, width: 1.2 }, rectRadius: 0.08
});
s1.addText('🏛️ PEMERINTAH KOTA SURAKARTA  •  BRIDA', {
  x: 0.9, y: 0.55, w: 4.6, h: 0.36,
  fontSize: 9.5, bold: true, color: C_CYAN_LIGHT, align: 'center', fontFace: FONT_HEAD
});

s1.addShape(pptx.shapes.ROUNDED_RECTANGLE, {
  x: 5.65, y: 0.55, w: 3.6, h: 0.36,
  fill: { color: C_BENTO_BG }, line: { color: C_AMBER, width: 1.2 }, rectRadius: 0.08
});
s1.addText('ROADMAP STRATEGIS 2026–2030', {
  x: 5.65, y: 0.55, w: 3.6, h: 0.36,
  fontSize: 9.5, bold: true, color: C_AMBER, align: 'center', fontFace: FONT_HEAD
});

s1.addShape(pptx.shapes.ROUNDED_RECTANGLE, {
  x: 9.40, y: 0.55, w: 3.033, h: 0.36,
  fill: { color: C_BENTO_BG2 }, line: { color: C_EMERALD, width: 1.2 }, rectRadius: 0.08
});
s1.addText('EDISI EKSEKUTIF RESMI', {
  x: 9.40, y: 0.55, w: 3.033, h: 0.36,
  fontSize: 9.2, bold: true, color: C_EMERALD, align: 'center', fontFace: FONT_HEAD
});

// Main Title
s1.addText('SOLO TECHNOPARK: DARI KAWASAN SAINS\nMENUJU MESIN IMPACT DAERAH', {
  x: 0.9, y: 1.15, w: 11.533, h: 1.65,
  fontSize: 32, bold: true, color: C_WHITE, fontFace: FONT_HEAD, lineSpacing: 36
});
...
s1.addText('"Roadmap Transformasi UPTD KST Surakarta: Mewujudkan Kemandirian BLUD, Hilirisasi Riset Kampus, dan Kesejahteraan Nyata Warga Kota"', {
  x: 0.9, y: 2.85, w: 11.533, h: 0.45,
  fontSize: 12.5, italic: true, color: C_AMBER_LIGHT, fontFace: FONT_BODY
});

// 4 Floating Metric Micro-Badges (V6 Enhancement)
const statPills = [
  { label: 'Lompatan Finansial', val: '+212,8% Mandiri', col: C_AMBER },
  { label: 'Target Pendapatan 2030', val: 'Rp 23,47 Miliar', col: C_CYAN_LIGHT },
  { label: 'Kapasitas Pelatihan', val: '9.000 SDM / Tahun', col: C_EMERALD },
  { label: 'Kemitraan Industri Global', val: '95+ Korporasi', col: C_PURPLE }
];

statPills.forEach((st, sidx) => {
  const sx = 0.9 + sidx * 2.92;
  s1.addShape(pptx.shapes.ROUNDED_RECTANGLE, {
    x: sx, y: 3.42, w: 2.80, h: 0.46,
    fill: 'FFFFFF', line: { color: st.col, width: 1 }, rectRadius: 0.04
  });
  s1.addText(`${st.label}: `, {
    x: sx + 0.12, y: 3.44, w: 1.45, h: 0.42,
    fontSize: 7.6, color: C_SILVER, fontFace: FONT_BODY, valign: 'middle'
  });
  s10Text:
  s1.addText(st.val, {
    x: sx + 1.40, y: 3.44, w: 1.30, h: 0.42,
    fontSize: 8.5, bold: true, color: st.col, fontFace: FONT_HEAD, align: 'right', valign: 'middle'
  });
});

// Horizontal Divider
s1.addShape(pptx.shapes.RECTANGLE, {
  x: 0.9, y: 4.02, w: 11.533, h: 0.02, fill: { color: C_BORDER }
});
...
coverCards.forEach((c, idx) => {
  ...
  addBentoCard(s1, cx, 4.18, c1w, 2.35, { fill: C_BENTO_BG, accentColor: c.accent });

  // Tag Badge
  s1.addText(`${c.icon}  ${c.tag}`, {
    x: cx + 0.20, y: 4.30, w: c1w - 0.4, h: 0.28,
    fontSize: 9.5, bold: true, color: c.accent, fontFace: FONT_HEAD
  });

  // Title
  s1.addText(c.title, {
    x: cx + 0.20, y: 4.58, w: c1w - 0.4, h: 0.32,
    fontSize: 12, bold: true, color: C_WHITE, fontFace: FONT_HEAD
  });

  // Desc
  s1.addText(c.desc, {
    x: cx + 0.20, y: 4.94, w: c1w - 0.4, h: 1.15,
    fontSize: 9.2, color: C_SILVER, fontFace: FONT_BODY, lineSpacing: 13
  });

  // Bottom Pill inside card
  s1.addShape(pptx.shapes.ROUNDED_RECTANGLE, {
    x: cx + 0.20, y: 6.16, w: c1w - 0.4, h: 0.26,
    fill: C_BENTO_BG2, line: { color: c.accent, width: 0.8 }, rectRadius: 0.03
  });
  s1.addText(`⭐ PILAR STRATEGIS: ${c.pill}`, {
    x: cx + 0.20, y: 6.16, w: c1w - 0.4, h: 0.26,
    fontSize: 7.8, bold: true, color: c.accent, align: 'center', fontFace: FONT_HEAD
  });
});
...
s1.addText('Disampaikan oleh: Kepala UPTD KST Solo Technopark  •  Didampingi: Kepala BRIDA Kota Surakarta  •  Surakarta, 2026', {
  x: 0.9, y: 6.94, w: 11.533, h: 0.30,
  fontSize: 8.8, color: C_SILVER, align: 'center', fontFace: FONT_BODY
});
...
const protoTableData = [
  [
    { text: 'KLASTER TEKNOLOGI', options: { bold: true, color: C_PURE_WHITE, fill: { color: C_NAVY_BAR } } },
    { text: 'CONTOH PRODUK INOVASI', options: { bold: true, color: C_PURE_WHITE, fill: { color: C_NAVY_BAR } } },
    { text: 'STATUS TAHAPAN', options: { bold: true, color: C_PURE_WHITE, fill: { color: C_NAVY_BAR }, align: 'center' } }
  ],
  [
...
const partnersTableData = [
  [
    { text: 'SEKTOR INDUSTRI', options: { bold: true, color: C_PURE_WHITE, fill: { color: C_NAVY_BAR } } },
    { text: 'MITRA STRATEGIS', options: { bold: true, color: C_PURE_WHITE, fill: { color: C_NAVY_BAR } } },
    { text: 'KONTRIBUSI NYATA BAGI KAWASAN STP', options: { bold: true, color: C_PURE_WHITE, fill: { color: C_NAVY_BAR } } }
  ],
  [
...
s6.addShape(pptx.shapes.ROUNDED_RECTANGLE, {
  ...
  fill: 'ECFDF5', line: { color: C_EMERALD, width: 1.5 }, rectRadius: 0.04
});
s6.addText('💎  PRINSIP ZERO APBD BURDEN: SELURUH FASILITAS CANGGIH DIBANGUN OLEH SWASTA', {
  x: 6.0, y: 5.42, w: 6.1, h: 0.28,
  fontSize: 9.2, bold: true, color: '047857', fontFace: FONT_HEAD
});
s6.addText('Laboratorium AI, Studio Gaming, 5G Experience, hingga Cyber Security Hub tidak dibangun dengan membebani belanja modal APBD Kota Surakarta. Semuanya hadir melalui kemitraan murni Public-Private Partnership (PPP).', {
  ...
  fontSize: 8.5, color: C_SILVER, fontFace: FONT_BODY, lineSpacing: 12
});
...
// V6 Upgrade: 5 Strategic Problem-to-Solution Breakthrough Action Cards
// =========================================================================
...
// Render 5 Problem-to-Solution Horizontal Rows
gapItems.forEach((g, gidx) => {
  const gy = 1.45 + gidx * 0.88;
  const gh = 0.80;

  // 1. Problem Card (Left: x: 0.8, w: 4.80)
  s7.addShape(pptx.shapes.ROUNDED_RECTANGLE, {
    x: 0.8, y: gy, w: 4.80, h: gh,
    fill: C_BENTO_BG, line: { color: g.isPriority ? C_AMBER : C_BORDER, width: g.isPriority ? 1.2 : 0.8 },
    rectRadius: 0.04
  });

  // Gap Badge
  s7.addShape(pptx.shapes.ROUNDED_RECTANGLE, {
    x: 0.92, y: gy + 0.10, w: 0.75, h: 0.24,
    fill: g.isPriority ? 'FEF3C7' : 'FEE2E2',
    line: { color: g.isPriority ? C_AMBER : C_RED, width: 0.8 }, rectRadius: 0.03
  });
  s7.addText(g.num, {
    x: 0.92, y: gy + 0.10, w: 0.75, h: 0.24,
    fontSize: 7.5, bold: true, color: g.isPriority ? C_AMBER_LIGHT : C_RED, align: 'center', fontFace: FONT_HEAD
  });

  // Gap Title
  s7.addText(g.name, {
    x: 1.75, y: gy + 0.08, w: 3.75, h: 0.26,
    fontSize: 8.8, bold: true, color: C_WHITE, fontFace: FONT_HEAD
  });

  // Gap Issue
  s7.addText(`Akar Masalah: ${g.issue}`, {
    x: 0.92, y: gy + 0.38, w: 4.58, h: 0.38,
    fontSize: 7.6, color: C_SILVER, fontFace: FONT_BODY, lineSpacing: 10
  });

  // 2. Connector Arrow (Center: x: 5.68, w: 0.34)
  s7.addShape(pptx.shapes.RIGHT_ARROW, {
    x: 5.68, y: gy + 0.25, w: 0.34, h: 0.28,
    fill: { color: g.isPriority ? C_AMBER : C_CYAN },
    line: { color: g.isPriority ? C_AMBER : C_CYAN }
  });

  // 3. Solution Card (Right: x: 6.10, w: 6.433)
  s7.addShape(pptx.shapes.ROUNDED_RECTANGLE, {
    x: 6.10, y: gy, w: 6.433, h: gh,
    fill: g.isPriority ? 'FEF3C7' : C_BENTO_BG2,
    line: { color: g.isPriority ? C_AMBER : g.accent, width: g.isPriority ? 1.5 : 0.8 },
    rectRadius: 0.04
  });
  ...
  // Solution Detail
  s7.addText(g.detail, {
    x: 6.25, y: gy + 0.38, w: 6.15, h: 0.38,
    fontSize: 7.8, color: C_WHITE, fontFace: FONT_BODY, lineSpacing: 10
  });
});

// Bottom Priority Callout (Quick Win Inkwal)
addBentoCard(s7, 0.8, 5.95, 11.733, 0.90, { fill: 'FFFBEB', line: C_AMBER, lineWidth: 1.5 });
s7.addText('⭐ PRIORITAS UTAMA #5: INSTRUKSI WALIKOTA e-KATALOG LOKAL AFIRMATIF (QUICK WIN 30 HARI)', {
  x: 1.0, y: 6.04, w: 11.333, h: 0.28,
  fontSize: 10, bold: true, color: C_AMBER_LIGHT, fontFace: FONT_HEAD
});
s7.addText('Target: Membuka captive market Rp 2,25 Miliar serapan belanja OPD Pemkot Surakarta di 2030 bagi 80+ produk tenant dan startup binaan STP tanpa perlu alokasi pagu belanja baru di APBD. Hanya butuh 1 lembar Instruksi Walikota.', {
  x: 1.0, y: 6.34, w: 11.333, h: 0.44,
  fontSize: 8.8, color: '78350F', fontFace: FONT_BODY, lineSpacing: 12
});
...
phases.forEach((ph, idx) => {
  const px = 0.8 + idx * 4.04;
  const pw = 3.65;
  addBentoCard(s8, px, 1.45, pw, 3.9, { fill: C_BENTO_BG, accentColor: ph.accent });

  // Phase Pill Header
  s8.addShape(pptx.shapes.ROUNDED_RECTANGLE, {
    x: px + 0.15, y: 1.6, w: pw - 0.30, h: 0.65,
    fill: C_BENTO_BG2, line: { color: ph.accent, width: 1 }, rectRadius: 0.05
  });
  s8.addText(ph.phase, {
    x: px + 0.2, y: 1.65, w: pw - 0.40, h: 0.25,
    fontSize: 9.5, bold: true, color: ph.accent, fontFace: FONT_HEAD, align: 'center'
  });
  s8.addText(ph.title, {
    x: px + 0.2, y: 1.9, w: pw - 0.40, h: 0.3,
    fontSize: 8.5, bold: true, color: C_WHITE, fontFace: FONT_HEAD, align: 'center'
  });

  // Target Box
  s8.addShape(pptx.shapes.ROUNDED_RECTANGLE, {
    x: px + 0.2, y: 2.35, w: pw - 0.40, h: 0.4,
    fill: C_DARK_BG, line: { color: C_BORDER, width: 1 }, rectRadius: 0.04
  });
  s8.addText(`Target: ${ph.target}`, {
    x: px + 0.2, y: 2.35, w: pw - 0.40, h: 0.4,
    fontSize: 9.5, bold: true, color: C_AMBER, fontFace: FONT_HEAD, align: 'center', valign: 'middle'
  });

  // Bullets
  const pList = ph.points.map(pt => `•  ${pt}`).join('\n\n');
  s8.addText(pList, {
    x: px + 0.2, y: 2.85, w: pw - 0.40, h: 2.35,
    fontSize: 9.0, color: C_SILVER, fontFace: FONT_BODY, lineSpacing: 12
  });

  // Transition Arrow between phases
  if (idx < 2) {
    s8.addShape(pptx.shapes.RIGHT_ARROW, {
      x: px + pw + 0.08, y: 1.76, w: 0.23, h: 0.32,
      fill: { color: C_AMBER }, line: { color: C_AMBER }
    });
  }
});
...
const mstoneData = [
  [
    { text: 'TAHUN 2026 (Baseline)', options: { bold: true, color: C_PURE_WHITE, fill: { color: C_NAVY_BAR }, align: 'center' } },
    { text: 'TAHUN 2027 (+67,0%)', options: { bold: true, color: C_PURE_WHITE, fill: { color: C_NAVY_BAR }, align: 'center' } },
    { text: 'TAHUN 2028 (+28,5%)', options: { bold: true, color: C_PURE_WHITE, fill: { color: C_NAVY_BAR }, align: 'center' } },
    { text: 'TAHUN 2029 (+28,0%)', options: { bold: true, color: C_PURE_WHITE, fill: { color: C_NAVY_BAR }, align: 'center' } },
    { text: 'TAHUN 2030 (MANDIRI)', options: { bold: true, color: '#FDE68A', fill: { color: C_NAVY_BAR }, align: 'center' } }
  ],
  [
...
s10.addShape(pptx.shapes.ROUNDED_RECTANGLE, {
  ...
  fill: 'E0F2FE', line: { color: '0284C7', width: 1 }, rectRadius: 0.04
});
...
s11.addChart(pptx.charts.BAR, impactChartData, {
  x: 0.95, y: 1.95, w: 4.95, h: 3.1,
  barDir: 'bar',
  chartColors: ['38BDF8', '0EA5E9', '10B981', 'F59E0B', 'F97316', '10B981'],
  chartColorsDiscardImageProps: true,
  showValue: true,
  dataLabelColor: '0F172A',
  dataLabelFontSize: 9,
  ...
  valAxisMaxVal: 110,
  showLegend: false,
  catGridLine: { color: 'E2E8F0', size: 1 },
  valGridLine: { style: 'none' }
});

// Bottom Callout Box inside Left Container
...
// Right Container: 6 Impact KPI Scorecard Cards in 2x3 Grid (w: 6.2)
const impactKPIs = [
  { icon: '👨‍🎓', title: 'SDM LEBIH KOMPETEN', value: '9.000 Peserta / Thn', sub: 'Kuota afirmasi 65% KTP Surakarta', col: C_CYAN_LIGHT },
  { icon: '🌱', title: 'UMKM & STARTUP TUMBUH', value: '80+ Wirausaha Baru', sub: 'Pendampingan & onboarding GoTo Hub', col: C_AMBER },
  { icon: '🔬', title: 'RISET DEKAT KE PASAR', value: '45+ Produk Hilirisasi', sub: 'Paten HKI, Uji NDT & Sertifikasi TKDN', col: C_EMERALD },
  { icon: '💻', title: 'AKSES TEKNOLOGI TERBUKA', value: '10.000+ Kunjungan/Thn', sub: 'Lab AI, Cyber & Prakerin SMK Solo Raya', col: C_CYAN },
  { icon: '💼', title: 'PELUANG KERJA BARU', value: 'Rp 180 M Multiplier', sub: 'Perputaran ekonomi hotel, resto & MICE', col: C_AMBER },
  { icon: '🤝', title: 'KOLABORASI INDUSTRI', value: '100% Investasi Swasta', sub: '95+ korporasi dunia tanpa beban APBD', col: C_EMERALD }
];

impactKPIs.forEach((k, idx) => {
  const col = idx % 2;
  const row = Math.floor(idx / 2);
  const cx = 6.3 + col * 3.1;
  const cy = 1.45 + row * 1.78;

  addBentoCard(s11, cx, cy, 2.95, 1.68, { fill: C_BENTO_BG, accentColor: k.col });

  s11.addText(`${k.icon}  ${k.title}`, {
    x: cx + 0.15, y: cy + 0.12, w: 2.65, h: 0.25,
    fontSize: 8.5, bold: true, color: C_WHITE, fontFace: FONT_HEAD
  });
  s11.addText(k.value, {
    x: cx + 0.15, y: cy + 0.45, w: 2.65, h: 0.55,
    fontSize: 13.5, bold: true, color: k.col, fontFace: FONT_HEAD, align: 'center', valign: 'middle'
  });
  s11.addText(k.sub, {
...
// =========================================================================
// SLIDE 12: 3 PERMOHONAN KEBIJAKAN & RENCANA AKSI 100 HARI (V6 Decision Badges)
// =========================================================================
...
// 3 Leadership Asks Horizontal Cards with Decision Badges
const asks = [
  {
  ...
  {
    badge: '🎓 KONSORSIUM RISET 5 KAMPUS SOLO RAYA',
    num: 'PERMOHONAN 3',
...
asks.forEach((a, idx) => {
  ...
  s12.addShape(pptx.shapes.ROUNDED_RECTANGLE, {
    x: ax + 0.15, y: 1.55, w: 3.5, h: 0.26,
    fill: C_DARK_BG, line: { color: a.accent, width: 0.8 }, rectRadius: 0.03
  });
  s12.addText(a.badge, {
    x: ax + 0.15, y: 1.55, w: 3.5, h: 0.26,
    fontSize: 7.2, bold: true, color: a.accent, fontFace: FONT_HEAD, align: 'center'
  });

  // Title Box
  s12.addShape(pptx.shapes.ROUNDED_RECTANGLE, {
    x: ax + 0.15, y: 1.88, w: 3.5, h: 0.46,
    fill: C_BENTO_BG2, line: { color: a.accent, width: 1 }, rectRadius: 0.04
  });
  s12.addText(`${a.num}: ${a.title}`, {
    x: ax + 0.2, y: 1.90, w: 3.4, h: 0.42,
    fontSize: 8.5, bold: true, color: C_WHITE, fontFace: FONT_HEAD, align: 'center', valign: 'middle'
  });

  // Description
  s12.addText(a.desc, {
    x: ax + 0.2, y: 2.38, w: 3.4, h: 0.85,
    fontSize: 8.5, color: C_SILVER, fontFace: FONT_BODY, lineSpacing: 11
  });

  // Target Box
  s12.addShape(pptx.shapes.ROUNDED_RECTANGLE, {
    x: ax + 0.2, y: 3.30, w: 3.4, h: 0.72,
    fill: C_DARK_BG, line: { color: C_BORDER, width: 1 }, rectRadius: 0.04
  });
  s12.addText(a.target, {
    x: ax + 0.25, y: 3.34, w: 3.3, h: 0.32,
    fontSize: 8.5, bold: true, color: C_WHITE, fontFace: FONT_HEAD
  });
  s12.addText(`⚡ ${a.status}`, {
    x: ax + 0.25, y: 3.68, w: 3.3, h: 0.28,
    fontSize: 8.0, italic: true, color: a.accent, fontFace: FONT_BODY
  });
});
...
const action100Data = [
  [
    { text: 'PERIODE', options: { bold: true, color: C_PURE_WHITE, fill: { color: C_NAVY_BAR }, align: 'center' } },
    { text: 'AGENDA STRATEGIS', options: { bold: true, color: C_PURE_WHITE, fill: { color: C_NAVY_BAR } } },
    { text: 'OUTPUT KONKRET & TARGET KEBERHASILAN', options: { bold: true, color: C_PURE_WHITE, fill: { color: C_NAVY_BAR } } }
  ],
  [
    { text: 'Hari 1–10', options: { align: 'center', color: C_CYAN_LIGHT, bold: true } },
    { text: 'Penyusunan Draf Instruksi Walikota e-Katalog', options: { color: C_WHITE } },
    { text: 'Draf Inkwal Afirmatif siap harmonisasi bersama Bagian Hukum Setda & BPKAD.', options: { color: C_SILVER } }
  ],
  [
    { text: 'Hari 11–30', options: { align: 'center', color: C_AMBER, bold: true } },
    { text: 'Harmonisasi & Penerbitan Instruksi Walikota', options: { color: C_AMBER, bold: true } },
    { text: 'Inkwal ditandatangani Walikota; Sosialisasi ke seluruh OPD prioritas Pemkot.', options: { color: C_WHITE, bold: true } }
  ],
  [
    { text: 'Hari 31–50', options: { align: 'center', color: C_CYAN_LIGHT, bold: true } },
    { text: 'Kurasi & Seleksi 5 Produk Inovasi Unggulan', options: { color: C_WHITE } },
    { text: 'Katalog 5 Produk Unggulan Hilirisasi STP (Smart Agri IoT, Alkes, Mesin Sortasi).', options: { color: C_SILVER } }
  ],
  [
    { text: 'Hari 51–60', options: { align: 'center', color: C_CYAN_LIGHT, bold: true } },
    { text: 'Showcase Inovasi di Depan Walikota & OPD', options: { color: C_WHITE } },
    { text: 'Demonstrasi produk di hadapan Walikota & media; komitmen serapan perdana OPD.', options: { color: C_SILVER } }
  ],
  [
    { text: 'Hari 61–80', options: { align: 'center', color: C_CYAN_LIGHT, bold: true } },
    { text: 'Inisiasi Konsorsium Riset 5 Perguruan Tinggi', options: { color: C_WHITE } },
    { text: 'MoU Pra-Konsorsium bersama Rektor UNS, UMS, ISI, Poltek Solo Raya.', options: { color: C_SILVER } }
  ],
  [
    { text: 'Hari 81–100', options: { align: 'center', color: C_EMERALD, bold: true } },
...
// WRITE TO DISK (V7)
// =========================================================================
const outputPptxPathV7 = path.resolve(__dirname, 'PAPARAN_WALIKOTA_STP_V7.pptx');
const outputPptxPathOut = path.resolve(__dirname, '../output_paparan/PAPARAN_WALIKOTA_STP_V7.pptx');

pptx.writeFile({ fileName: outputPptxPathV7 })
  .then(() => {
    console.log(`[SUCCESS] PPTX Master V7 generated at: ${outputPptxPathV7}`);
    fs.copyFileSync(outputPptxPathV7, outputPptxPathOut);
    console.log(`[SUCCESS] Copied to output_paparan: ${outputPptxPathOut}`);
  })
  .catch(err => {
    console.error('[ERROR] Failed generating PPTX V7:', err);
    process.exit(1);
  });
